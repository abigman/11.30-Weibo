...
- (void)setupNavigationBarItem {
  ...
  self.navigationItem.leftBarButtonItem = [UIBarButtonItem
      itemWithImageName:@"navigationbar_friendsearch"
       hightedImageName:@"navigationbar_friendsearch_highlighted"
                 target:self
                 action:@selector(friendsSearch)];

  // 右边按钮
  self.navigationItem.rightBarButtonItem =
      [UIBarButtonItem itemWithImageName:@"navigationbar_pop"
                        hightedImageName:@"navigationbar_pop_highlighted"
                                  target:self
                                  action:@selector(pop)];
  ...
  NSString *titleName = [YCLAccountTool readAccount].name
                            ? [YCLAccountTool readAccount].name
                            : @"首页";
  [titleButton setTitle:titleName forState:UIControlStateNormal];
  [titleButton setImage:[UIImage imageNamed:@"navigationbar_arrow_down"]
               forState:UIControlStateNormal];
  [titleButton
      setBackgroundImage:
          [UIImage resizableImageNamed:@"navigationbar_title_highlighted"]
                forState:UIControlStateHighlighted];
  [titleButton addTarget:self
                  action:@selector(titleButtonOnClicked:)
...
- (void)titleButtonOnClicked:(UIButton *)sender {
  [sender setImage:[UIImage imageNamed:@"navigationbar_arrow_up"]
          forState:UIControlStateNormal];
...
- (void)requestUserInfo {
  YCLUserRequestParameter *requestParas =
      [[YCLUserRequestParameter alloc] init];
  ...
  [YCLUserRequestTool userWithParameters:requestParas
      success:^(YCLUserResponseResult *responseResult) {
...
- (void)loadMoreNewerStatus:(UIRefreshControl *)refreshControl {
  NSLog(@"加载新微博");
  YCLStatusRequestParameter *requestParas =
      [[YCLStatusRequestParameter alloc] init];
  ...
  [YCLStatusTool statusWithParameters:requestParas
      success:^(YCLStatusResponseResult *responseResult) {
        NSLog(@"微博请求成功");
        NSArray *addedNewStatusFrames =
            [self statusFramesWithStatuses:responseResult.statuses];
        // 新微博应该添加到最前面
        NSIndexSet *indexSet = [NSIndexSet
            indexSetWithIndexesInRange:NSMakeRange(0,
                                                   addedNewStatusFrames.count)];
        [self.statusFrames insertObjects:addedNewStatusFrames
                               atIndexes:indexSet];
...
- (void)addRefreshControl {
  ...
  [refreshControl addTarget:self
                     action:@selector(loadMoreNewerStatus:)
...
- (void)showOldStatusCount:(int)count {
  ...
  label.backgroundColor = [UIColor orangeColor];
  label.alpha = 0;
  [self.tabBarController.view insertSubview:label
                               belowSubview:self.tabBarController.tabBar];
  [UIView animateWithDuration:0.5
      animations:^{
        label.alpha = 1;
        label.frameY -= 22;
...
- (void)popMenuDidDismiss:(YCLPopMenu *)popMenu {
  // 获取 标题按钮
  YCLHomeTitleButton *titleButton =
      (YCLHomeTitleButton *)self.navigationItem.titleView;
  // 设置箭头向下
  [titleButton setImage:[UIImage imageNamed:@"navigationbar_arrow_down"]
               forState:UIControlStateNormal];
}
